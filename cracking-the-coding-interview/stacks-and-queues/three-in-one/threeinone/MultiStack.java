/**.
 * File: MultiStack.java
 * ---------------------
 * A second approach is to allow the stack blocks to be flexible
 * in size. When one stack exceeds its initial capacity, we grow
 * the allowable capacity and shift elements as necessary.
 *
 * We will  also design our array to be circular, such that the
 * final stack may start at the end of the array and wrap around
 * to the beginning.
 */
package threeinone;
...
public class MultiStack {
     ...
     * We could have done thos with just a bunch of individual
    ...
    private int adjustIndex(int index) {
         ...
         * (-11 % 5) will return -1, not 4. We actually want the value
         * to be 4 (since we 're wrapping around the index) */
    ...
    private int nextIndex(int index) {
        return adjustedIndex(index + 1);
    }

    /* get index before this index, adjusted for warap around */
    ...
    private void shift(int stackNum) {
        System.out.print("/// Shifting " + stackNum + "\n");
        ...
        /* shift all elements in stack over by one. */
         int index = stack.lastCapactyndex();
         while (stack.sWthStackCapacty(index)) {
    ...
    /* expand stack by shifing over other stacks */
    private void expand(int stackNum) {
        System.out.print("///Expanding stack " + stackNum + "\n");
    ...
    public void push(int stackNum, int value) throws FullStackException {
        System.out.print("/// Pushing stack " + stackNum + ": " + value + "\n");
        if (allStacksAreFull()) {
    ...
    public int pop(int stackNum) throws Exception {
        System.out.print("/// Popping stack " + stackNum + "\n");
        StackInfo stack = info[stackNum];
        if (stack.isEmpty()) {
     ...
     * @param arr is the arr to be converted to string
     * @return return a string representation of arr.
     */
